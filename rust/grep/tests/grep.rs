...
static MIDSUMMER_NIGHT_CONTENT: &'static str =
...
static PARADISE_LOST_CONTENT: &'static str =
...
fn tear_down_files(files: &[&str]) {
    for file_name in files {
        fs::remove_file(file_name).unwrap_or_else(|_| {
            panic!("Could not delete file '{}'", file_name)
        });
    }
}
...
set_up_test_case!(
    ...
    test_multiple_files_one_match_multiple_flags(
        ...
        files = ["iliad.txt", "midsummer_night.txt", "paradise_lost.txt"],
        prefix_expected =
            ["paradise_lost.txt:4:With loss of Eden, till one greater Man"]
    )
);
