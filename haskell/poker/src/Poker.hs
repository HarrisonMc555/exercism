...
instance Ord Score where
  compare (Score part1 rem1) (Score part2 rem2) =
    case compare part1 part2 of
      EQ    -> if rem1 == rem2
               then EQ
               else compare (score rem1) (score rem2)
      other -> other
...
score :: Hand -> Score
score hand =
  let bestScore = firstJusts . map ($ hand) $ handScorers
...
royalFlush hand =
  do highRank <- highestRankWhenInARow hand
...
fourOfAKind hand =
  do rankWithFour <- nOfAKind 4 hand
...
fullHouse hand =
  do rankWithThree <- nOfAKind 3 hand
...
twoPair hand =
  do rank1 <- nOfAKind 2 hand
     let handWithoutRank1 = removeNMatching 2 ((== rank1) . rank) hand
...
highCard hand =
  do highRank <- maximumMay $ map rank hand
...
highestRankWhenInARow hand =
  do guard $ isFullHand hand
     highWhenAllInRow hand
...
highWhenAllInRow :: Hand -> Maybe Rank
highWhenAllInRow hand@[_, _, _, _, _]
  | inARowNormally = Just lastRank
  | inARowWithAceFirst = Just highOfFour
  | otherwise = Nothing
  where ranks = sort . map rank $ hand
        inARowNormally = isSucc ranks
        (rest, lastRank) = fromJust . splitLast $ ranks
        inARowWithAceFirst =
          isSucc rest &&
          (listToMaybe rest == Just Two) &&
          (lastRank == Ace)
        highOfFour = ranks !! 3
highWhenAllInRow _ = Nothing
