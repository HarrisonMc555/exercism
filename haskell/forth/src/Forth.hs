...
data ForthMode = Normal
               | StartingCustomWord
               | DefiningCustomWord Text [ForthCommand]

emptyState :: ForthState
emptyState = ForthState { stack = [], custom = initialCustomCommands, mode = Normal }

evalText :: Text -> ForthState -> Either ForthError ForthState
evalText text state =
     do let textWords = Text.words text
...
parseAndEvalCommand state text =
     do token <- parseToken text
     ...
     case mode' of
               ...
               case token of
                    Custom text ->
                         case parseWord text of
                              Just name -> Right $ ForthState stack' custom' $ DefiningCustomWord name []
          ...
          DefiningCustomWord name commands ->
               case token of
                    ...
                    EndCustom -> Right $ ForthState stack' (Map.insert name commands custom') Normal
                    _ -> do command <- tokenToCommand custom' token
                            Right $ ForthState stack' custom' $ DefiningCustomWord name (command ++ commands)
          ...
          Normal ->
               case token of
                    ...
                    _ -> do commands <- tokenToCommand custom' token
...
parseNumber text =
     case Read.decimal text of
          Right (x, rest) | Text.null rest -> Just $ Number x
          _ -> Nothing
...
tokenToCommand :: Map Text [ForthCommand] -> ForthToken -> Either ForthError [ForthCommand]
...
initialCustomCommands =
     Map.fromList . map (uncurry convert) $ builtins
     where
          convert name f =
               (name, [Builtin f])
...
builtins =
     ...
     where
          ...
          sub = wrap (-)
          mul = wrap (*)
          div' (x1:x2:rest) = if x1 == 0 then Left DivisionByZero else Right $ x2 `div` x1 : rest
